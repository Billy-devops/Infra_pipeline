...
#| ----------------------------------------------- | ------------------- | -------------------------------------------------- |
#| Load Balancer Terraform se create hua           | Direct reference    | `loadbalancer_id = azurerm_lb.testlb[each.key].id` |
#| Load Balancer manually Azure me bana            | Data block use karo | `data.azurerm_lb.existing_lb.id`                   |
#| Load Balancer manually bana but import kar liya | Direct reference    | `azurerm_lb.testlb.id`                             |
